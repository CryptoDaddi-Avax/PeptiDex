...
import {
  ...
  FlaskConical, CheckCircle2, Shield, Activity, FileText, ArrowUpRight
} from 'lucide-react';

// The 5 specific peptides requested
...
export function generateMetadata({ params }: { params: { peptide: string } }): Metadata {
  ...
  // Custom metadata per slug for buyer intent
  let title = '';
  let description = '';
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://peptidex.app/vendors/amino-club/${slug}`,
      type: 'article',
    },
  };
}
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 md:py-12 relative space-y-12">
      ...
      <header className="space-y-6">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="px-3 py-1 text-[10px] font-bold uppercase tracking-widest bg-zinc-800 text-zinc-300 rounded-full border border-zinc-700">
            {pData.category}
          </span>
        </div>
        <h1 className="text-3xl md:text-4xl lg:text-5xl font-extrabold tracking-tight text-zinc-100 leading-tight">
          Sourcing <span className="text-emerald-400">{name}</span> from Amino Club
        </h1>
      ...
      {/* QUICK BUY BOX */}
      <div className="bg-zinc-900 border-2 border-emerald-500/30 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden shadow-xl shadow-emerald-900/10">
        ...
        <div className="relative z-10 w-full md:w-auto">
            ...
            {aminoClubPricing ? (
                <div className="text-3xl font-black text-emerald-400 mb-1">
                    ${aminoClubPricing.price_usd.toFixed(2)} <span className="text-sm text-zinc-500 font-normal tracking-wide">/ {aminoClubPricing.vial_mg}mg vial</span>
                </div>
            ) : (
        ...
        <div className="relative z-10 w-full md:w-auto flex flex-col items-center md:items-end">
            ...
            <p className="mt-3 text-sm text-zinc-300 font-medium bg-zinc-950 border border-zinc-800 px-4 py-1.5 rounded-lg w-full md:w-auto text-center">
                Use code <strong className="text-emerald-400 font-mono tracking-wider ml-1">PEPTIDEX</strong> for 15% off
            </p>
        </div>
      </div>
      ...
      <article className="prose prose-invert prose-zinc max-w-none prose-headings:font-bold prose-h2:text-2xl prose-h2:text-zinc-200 prose-h2:mt-12 prose-h2:mb-6 prose-h2:pb-2 prose-h2:border-b prose-h2:border-zinc-800 prose-p:text-zinc-300 prose-p:leading-loose prose-a:text-emerald-400 prose-a:no-underline hover:prose-a:underline prose-strong:text-zinc-200 prose-ul:text-zinc-300 prose-li:marker:text-emerald-500">
        ...
        <h2 id="why-amino-club">Why Source {name} from Amino Club?</h2>
        <p>
          When researching <strong>{name}</strong>, verifying the identity and purity of the compound is the most critical step. Due to its popularity in {pData.category.toLowerCase()} studies, the market is flooded with under-dosed or entirely counterfeit vials originating from unverified overseas suppliers.
        </p>
        <p>
          We recommend sourcing {name} from Amino Club for three distinct reasons:
        </p>
        <ul>
            <li><strong>Batch-Specific Testing:</strong> Amino Club utilizes independent laboratories (like MZ Biolabs) to perform HPLC and Mass Spectrometry on <em>every single batch</em> of {name} they synthesize. They do not rely on outdated "golden batch" COAs.</li>
            <li><strong>Purity Standards:</strong> Their baseline acceptable purity is &ge;99%. When reviewing their current testing documents for {name}, the chromatograms consistently show exceptional purity with virtually zero truncated sequences.</li>
            <li><strong>Domestic Fulfillment:</strong> They ship from within the United States. This eliminates the weeks-long delays and customs seizure risks associated with ordering {name} directly from Chinese laboratories.</li>
        </ul>

        <h2 id="coa-verification">COA Verification for {name}</h2>
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl my-6 not-prose flex flex-col md:flex-row gap-6 items-start md:items-center">
            ...
            <div className="flex-1">
                <h4 className="text-lg font-bold text-zinc-100 mb-1">Independent Laboratory Analysis</h4>
        ...
        <h2 id="pricing-value">Pricing & Value Analysis</h2>
        <p>
          At <strong>${aminoClubPricing?.price_usd.toFixed(2)}</strong> for a {aminoClubPricing?.vial_mg}mg vial, Amino Club offers highly competitive pricing for COA-verified {name}.
        </p>
        <p>
        ...
        <p>
          Furthermore, when you apply the <code>PEPTIDEX</code> discount code at checkout, Amino Club's pricing often beats other premium, verified US-based vendors like Limitless Life and Ascension Peptides. They also offer bulk pricing tiers (e.g., 5-packs or 10-packs) which stack with the discount code for heavy researchers.
        </p>

        <h2 id="who-should-consider">Who Should Consider {name}?</h2>
        <p>
        ...
        <p>
          <strong>Dosing & Administration:</strong> If you are planning a research protocol with {name}, you can refer to our comprehensive <Link href={`/library/${slug}`} className="text-emerald-400 font-semibold hover:underline">{name} Research Profile</Link> for aggregated data on clinical half-life ({pData.half_life_hours} hours), receptor affinity, and standard investigational dosing parameters.
        </p>
        ...
        <div className="not-prose mt-12 p-8 rounded-2xl border border-emerald-500/30 bg-gradient-to-br from-emerald-900/20 to-zinc-900 text-center">
            ...
            <p className="text-zinc-300 mb-6">
                Purchase verified {name} and use our exclusive discount code to save 15% on your entire Amino Club order.
            </p>
      ...
      <div className="mt-16 p-6 rounded-2xl bg-zinc-900 border border-zinc-800 flex flex-col gap-2">
        ...
        <p className="text-sm text-zinc-400 leading-relaxed italic">
          This article is for informational purposes only and does not constitute medical advice. {name} is sold strictly for in-vitro laboratory research and is not intended for human consumption. PeptiDex may earn a commission from purchases made through affiliate links.
        </p>
      </div>
